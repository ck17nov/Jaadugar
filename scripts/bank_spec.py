...
def build(only_group: str) -> str:
    ...
    w("")
    w("**These are shares of the whole GROUP AND LANGUAGE, not of your")
    w("topic and not of your batch.** `kids` + `hi` is one pool: every")
    w("Hindi kids entry already banked, across every topic, counts against")
    w(f"your {int(variety.MAX_ARC_SHARE * 100)}%. The bank is not empty, so")
    w("treat the numbers above as a ceiling and aim well under them.")
    w("")
    w("Doing this correctly in one language does NOT cover the other. The")
    w("third real batch is the proof - same topic, two languages, one")
    w("brief:")
    w("")
    w("| cell | `alone` | outcome |")
    w("|---|---|---|")
    w("| kids / **en** / toys and play | 6 of 45, spread across all 8 "
      "arcs | **45 of 45 landed** |")
    w("| kids / **hi** / toys and play | **38 of 50 (76%)** | 11 refused "
      "on `arc_share` |")
    w("")
    w("Nothing was wrong with those 11 scripts. Count your own arcs before")
    w("handing the file over:")
    w("")
    w("```")
    w("python -c \"import json,sys,collections;"
      "print(collections.Counter(json.loads(l).get('arc_variant') "
      "for l in open(sys.argv[1],encoding='utf-8') if l.strip()))"
      "\" your-file.jsonl")
    w("```")
    w("")
    w("If any single value is near a fifth of the file, rewrite some")
    w("before submitting - free to change now, impossible to recover")
    w("later.")
    w("")
    ...
    w("")
    w("**Do not lead the sentence with the perception.** Four entries in")
    w("the third batch read \"Tara noticed the grooved feet on each")
    w("figure, interlocking them into one tall tower\" - the invention is")
    w("there, but it arrives after the looking. Put the action first:")
    w("\"Tara interlocked the grooved feet into one tall tower.\" It reads")
    w("better and cannot be misread as a turn where the child only")
    w("looked.")
    w("")
    ...
    w("| `shoot` alone | reads as violence | `green shoot`, `new shoots`, "
      "and `shoot` beside `camera`/`lens`/`photo` are allowed |")
    ...
    w("| `dead`, `die` | violence, unless the subject is a torch, lamp, "
      "battery or flame; `dead weight`, `dead end` and `stopped dead` are "
      "allowed | `the torch went out` |")
    w("| `rifle`, `pistol`, `revolver`, `shotgun`, `firearm`, `grenade`, "
      "`sniper`, `dagger`, `machete` | **blocked outright, no exceptions** "
      "- these were missing from the check entirely | leave weapons out of "
      "children's content |")
    ...
    w("")
    w("Malformed lines: three in the first real batch, two in the second,")
    w("and **five in the third**. The third had one repeatable cause worth")
    w("naming, because it is easy to produce and easy to miss - a scene")
    w("object whose `narration` KEY is missing while its value is still")
    w("there:")
    w("")
    w("```")
    w("{\"beat\":\"turn\",\"<the narration text>\",\"caption\":...")
    w("                   ^ broken - no key")
    w("{\"beat\":\"turn\",\"narration\":\"<the narration text>\","
      "\"caption\":...   correct")
    w("```")
    w("")
    w("Those five were recovered, because the key order is identical in")
    w("all 940 well-formed scenes so the missing name was certain. Do not")
    w("rely on that - a break anywhere else, or a truncated line, loses")
    w("the entry. Before handing the file over, check every line parses:")
    w("")
    w("```")
    ...
    w("```")
    w("")
    w("Silence means every line is good.")
    w("")
    w("### 6. No two entries may share the same narration")
    w("")
    w("An entry's identity is a hash of its narration text, nothing else -")
    w("not the title, not the topic, not the `on_screen_text`. Two entries")
    w("with byte-identical narration are therefore ONE entry, and the")
    w("second silently replaces the first.")
    w("")
    w("The third batch lost 10 more - 145 lines holding only 135 distinct")
    w("scripts, each repeat under a fresh title.")
    w("")
    w("The second real batch lost 15 entries this way: 14 speaking drills")
    w("repeated verbatim under different titles, and one pair where two")
    w("different Hindi spelling words were given the same script body. A")
    w("different title does not make a different entry. Check it:")
    w("")
    w("```")
    w("python -c \"import json,sys,collections;"
      "c=collections.Counter(chr(10).join(s['narration'] for s in "
      "json.loads(l)['scenes']) for l in open(sys.argv[1],encoding='utf-8') "
      "if l.strip());print([n for n,k in c.items() if k>1][:5] or 'all unique')"
      "\" your-file.jsonl")
    w("```")
    w("")
    w("Each `refrain` must be unique too - not just against the bank, but")
    w("within your own batch.")
    w("")
    w("### 7. The rejection that was not a gate failure at all")
    w("")
    w("1015 entries were reviewed. 893 were kept and **122 were rejected in")
    w("one sentence: \"not up to the mark, not engaging\".** Every one of")
    w("those 122 had already passed schema, story shape, the obstacle and")
    w("turn checks, safety, variety and the share caps. So sections 8 and 9")
    w("are not new taste - they are the habits the rejected batch had and")
    w("the 893 kept entries do not.")
    w("")
    w("**Read the honesty note before you weight any of it.** The 122 were")
    w("one automated batch from one tool, and the verdict was a single")
    w("sentence over the whole file - there is ONE negative label, not 122.")
    w("Worse, every rejected entry came from that tool and no kept entry")
    w("did, so \"rejected\" and \"written by that tool\" are the same")
    w("variable. Measuring the two sets against each other finds the tool's")
    w("fingerprints as easily as any flaw: a space before the percent sign")
    w("(\"42 %\") appears in 12 of the 122 and 0 of the 893, and it has")
    w("nothing to do with whether a script is engaging.")
    w("")
    w("That is why almost nothing from that comparison became a gate, and")
    w("why section 8 is GUIDANCE with its numbers shown rather than a rule")
    w("you can be refused for. Section 9 is enforced, because it is an")
    w("entry contradicting itself rather than a matter of style.")
    w("")
    w("### 8. Rhythm - GUIDANCE, not enforced")
    w("")
    w("Four mechanical habits. Each one is measured against the whole bank,")
    w("and the point of quoting the kept column is that following these")
    w("cannot hurt you - 891 of the 893 kept entries already satisfy all")
    w("four:")
    w("")
    w("| | rule | rejected batch | kept bank |")
    w("|---|---|---|---|")
    w("| 8a | at least one scene carries **two or more sentences** | "
      "46/122 broke it | **0/893** |")
    w("| 8b | longest sentence at least **6 words longer** than the "
      "shortest *(poems exempt - metre is the point)* | 43/122 | 2/893 |")
    w("| 8c | mean narration of **9+ words per scene**, 11 is better | "
      "61/122 | **0/893** |")
    w("| 8d | under **40% of scenes at 8 words or shorter** "
      "*(poems exempt)* | 55/122 | **0/893** |")
    w("")
    w("Together they flag 93 of the 122 rejected entries (76%) and 2 of the")
    w("893 kept ones (0.2%). The short version: **the rejected batch was")
    w("chopped into single short sentences of one length.** Vary your")
    w("sentence lengths inside an entry and let a scene carry two thoughts.")
    w("")
    w("Two findings are recorded here because they are the opposite of what")
    w("you would guess, and writing to them would make a batch worse:")
    w("")
    w("- **A repeated opening formula is not the problem.** All 181 of the")
    w("  kept English kids drills begin with the identical three words")
    w("  (\"Today we will\") - the most formulaic opening in the whole")
    w("  dataset sits in the corpus that was kept.")
    w("- **Short openings are not the problem either.** A kept corpus opens")
    w("  shorter than the rejected one in both drill cells (median 6 and 5")
    w("  words against 7).")
    w("")
    w("One thing to avoid outright, for tech: **do not open on a number.**")
    w("A percentage in the first spoken line appears in 13 of the 122 and 0")
    w("of the 893; a spec figure of any kind in 25 of 122 (39% of tech")
    w("explainers) against 2 of 276. Note the system's own hook scorer")
    w("REWARDS a digit in the opening - it is calibrated for a different")
    w("kind of video, and on this batch it scored the rejected entries")
    w("higher than the kept ones. Ignore it and open on the thing itself.")
    w("")
    w("### 9. A refrain has to BE a refrain - ENFORCED, every shape")
    w("")
    w("Sections above ask for a verbatim refrain in narrative and poem. It")
    w("is now checked for **any** shape that fills the `refrain` field,")
    w("because drills were never asked and it showed: 11 of the 32 rejected")
    w("entries with a declared refrain spoke it ONCE, against 0 of the 721")
    w("kept ones. A line said once is not a refrain.")
    w("")
    w("> **Blocking:** if you fill `refrain`, that exact string must appear")
    w("> in the narration of **at least two scenes** - and **three** if")
    w("> the shape is `narrative` or `poem`, where the older story gate has")
    w("> always required three and still does. Two is the floor for every")
    w("> other shape. If the shape has no")
    w("> refrain, leave the field empty - an empty field is never checked.")
    w("")
    w("**Do not let the refrain grade the child.** All 10 rejected Hindi")
    w("drills had a refrain containing सही or "
      "बिल्कुल - "
      "\"क क क, बिल्कु"
      "ल सही!\" (\"k k k, absolutely correct!\") -")
    w("against 0 of the 721 kept. The refrain is the line the child says")
    w("ALONG WITH the narrator, so a refrain asserting the answer was right")
    w("lands before they have answered. This one warns rather than blocks:")
    w("a celebration line after the answer is a fine choice, and only using")
    w("it as the repeated refrain is the mistake.")
    w("")
    w("## The fields")
    w("")
