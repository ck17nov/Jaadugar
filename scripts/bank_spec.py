...
def build(only_group: str) -> str:
    ...
    w("")
    w("Three lines of the first real batch were malformed, and two of the")
    w("second - a delimiter broken part way through a long object, so a")
    w("string closes early or a value has no key. They cannot be repaired")
    w("without inventing the missing content, so they are dropped. Before")
